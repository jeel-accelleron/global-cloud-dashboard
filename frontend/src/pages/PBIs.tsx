...
const BUCKETS: Bucket[] = [
  {
    key: 'new',
    label: 'New PBIs',
    states: ['New', 'Approved', 'To Do', 'Proposed'],
    tone: 'info',
    color: '#3b82f6',
    icon: CircleDashed,
  },
  {
    key: 'committed',
    label: 'Committed PBIs',
    states: ['Committed', 'Active', 'In Progress', 'Doing'],
    tone: 'warning',
    color: '#f59e0b',
    icon: ListChecks,
  },
  ...
  return (
    <Card className="h-full">
      ...
      <CardBody className="p-0">
        {sorted.length === 0 ? (
          <EmptyState title="Nothing here" description="No PBIs in this bucket." />
        ) : (
  ...
  const grouped = useMemo(() => {
    ...
    for (const it of data) {
      ...
      const match = BUCKETS.find((b) =>
        b.states.some((bs) => bs.toLowerCase() === s)
      );
      if (match) out[match.key].push(it);
    }
    return out;
  }, [data]);
  ...
  return (
    <>
      <PageHeader
        ...
        description="PBI breakdown by status, assignee, and recency"
      />
